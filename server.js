...
const COINS_PER_DOLLAR = 100; // 100 coins = $1
...
async function router(req, res) {
  try {
    ...
    if (m === 'POST' && p === '/api/stripe/create-checkout') {
      const user = getUser(authToken(req));
      if (!user) return send(res, 401, { error: 'Not logged in' });
      if (!STRIPE_SECRET) return send(res, 503, { error: 'Payments not configured. Set STRIPE_SECRET_KEY env var.' });
      const { package: pkg } = await readBody(req);
      const packages = { starter: { coins: 500, amount: 500 }, popular: { coins: 1200, amount: 1000 }, premium: { coins: 3000, amount: 2000 } };
      const chosen = packages[pkg];
      if (!chosen) return send(res, 400, { error: 'Invalid package' });
      // Call Stripe API
      const stripeRes = await stripePost('/v1/checkout/sessions', {
        payment_method_types: ['card'],
        line_items: [{ price_data: { currency: 'usd', product_data: { name: `NUMSE ${chosen.coins} Coins` }, unit_amount: chosen.amount }, quantity: 1 }],
        mode: 'payment',
        success_url: `${req.headers.origin || 'http://localhost:'+PORT}/?payment=success&coins=${chosen.coins}&user=${user.username}`,
        cancel_url: `${req.headers.origin || 'http://localhost:'+PORT}/?payment=cancelled`,
        metadata: { username: user.username, coins: String(chosen.coins) }
      });
      if (stripeRes.error) return send(res, 500, { error: stripeRes.error.message || 'Stripe error' });
      return send(res, 200, { url: stripeRes.url });
    }
    ...
    if (m === 'POST' && p === '/api/stripe/webhook') {
      ...
      if (body.type === 'checkout.session.completed') {
        const { username, coins } = body.data?.object?.metadata || {};
        if (username && coins && db.users[username]) {
          db.users[username].coins += Number(coins);
          db.transactions.push({ type: 'purchase', username, coins: Number(coins), at: Date.now() });
        }
      }
...
function stripePost(endpoint, data) {
  return new Promise((resolve) => {
    const body = new URLSearchParams(flattenForStripe(data)).toString();
    const options = {
      hostname: 'api.stripe.com', port: 443, path: endpoint, method: 'POST',
    ...
    const reqS = https.request(options, r => {
      let d = '';
      r.on('data', c => d += c);
      r.on('end', () => { try { resolve(JSON.parse(d)); } catch { resolve({ error: { message: 'Parse error' } }); } });
    });
    reqS.on('error', e => resolve({ error: { message: e.message } }));
    reqS.write(body); reqS.end();
  });
}
function flattenForStripe(obj, prefix = '') {
  ...
  for (const [k, v] of Object.entries(obj)) {
    ...
    if (v && typeof v === 'object' && !Array.isArray(v)) Object.assign(result, flattenForStripe(v, key));
    else if (Array.isArray(v)) v.forEach((item, i) => { if (typeof item === 'object') Object.assign(result, flattenForStripe(item, `${key}[${i}]`)); else result[`${key}[${i}]`] = item; });
    else result[key] = v;
  }
  return result;
}
...
http.createServer(router).listen(PORT, () => {
  ...
  if (!STRIPE_SECRET) console.log(`  ⚠️  Stripe not configured. Set STRIPE_SECRET_KEY to enable payments.\n`);
  else console.log(`  ✅  Stripe payments enabled.\n`);
});
