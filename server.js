...
function loadDB() {
  ...
  return {
    users: {
      ...
      admin: { username: 'admin', password: hash('admin123'), coins: 99999, joinedAt: Date.now() }
    },
    listings: [], 
    transactions: []
  };
...
// Higher limit for image uploads
function readBody(req) {
  return new Promise((resolve) => {
    let data = '';
    req.on('data', chunk => {
        data += chunk;
        if (data.length > 10 * 1024 * 1024) req.destroy(); // 10MB Limit
    });
    req.on('end', () => { 
...
async function router(req, res) {
  ...
  if (method === 'POST' && pathname === '/api/login') {
    ...
    if (!user || user.password !== hash(password)) return send(res, 401, { error: 'Invalid' });
  ...
  if (method === 'GET' && pathname === '/api/listings') {
    return send(res, 200, db.listings);
  }
  ...
  if (method === 'POST' && pathname === '/api/listings') {
    ...
    const item = {
      ...
      image: body.image, // Base64 string from frontend
      createdAt: Date.now()
    };
  ...
  if (method === 'POST' && pathname === '/api/admin/give-coins') {
    ...
    if (db.users[target]) {
        db.users[target].coins += Number(amount);
        saveDB();
...
http.createServer(router).listen(PORT, () => {
  console.log(`Marketplace live on ${PORT}`);
});